    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text(code.uppercased())
                .font(.system(.largeTitle, design: .monospaced)).bold().tracking(4)
            HStack {
                #if os(macOS)
...
private struct FullPlayerView: View {
    ...
    var body: some View {
        ...
        #if os(tvOS)
        // Siri Remote: play/pause toggles playback; Menu minimizes the player
        // instead of backing out of the app.
        .onPlayPauseCommand { model.togglePlayPause() }
        .onExitCommand { model.minimizePlayer() }
        #endif
    }

    // Top bar: title plus track/quality/queue controls.
    private var controlBar: some View {
        HStack(spacing: 24) {
            Button { model.minimizePlayer() } label: { Image(systemName: "chevron.down") }

            Text(model.nowPlayingTitle ?? "").font(.headline).lineLimit(1)
            Spacer()

            Button { model.presentInfo() } label: { Image(systemName: "info.circle") }
            ...
            Button { model.showQueue = true } label: { Image(systemName: "line.3.horizontal") }
    ...
    // Bottom bar: progress plus focusable transport buttons. The TV build has
    // no slider control, so seeking happens through the skip buttons.
    private var transportBar: some View {
        VStack(spacing: 16) {
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        Capsule().fill(Color.white)
                            .frame(width: model.duration > 0
                                   ? geo.size.width * min(max(model.currentTime / model.duration, 0), 1)
                                   : 0)
                    }
                }
                .frame(height: 6)
            ...
            HStack(spacing: 40) {
                ...
                Button { model.skip(by: -60) } label: { Image(systemName: "gobackward.60") }
                ...
                Button { model.skip(by: 60) } label: { Image(systemName: "goforward.60") }
        ...
        .foregroundStyle(.white)
        .padding(.horizontal, 48).padding(.top, 16).padding(.bottom, 28)
        .background(
